...
#Checking how to make the legend(Mchi/Mdm equal or g's equal(depending on readin files))
...
def make_plot(counter, num, tree, information):
	...
	if counter == 1:	
		leg_list[num] = TLegend(.60, .82-leg_size, .73, .85)
	...
	g_q = "Gq "+str(round(information.Gq*1000)/1000.) #Issue with root float storage.
	g_dm = "Gdm "+str(round(information.Gdm*1000)/1000.)
	g_both = g_dm

	if set_phi and set_chi:
		leg_list[num].SetHeader(" Model: #splitline{ "+m_chi+" GeV}{ "+m_phi+" Gev} "+g_dm)  #Special for gdm == 1
		leg_list[num].AddEntry(tree, g_q, "l")
